...
window.onload = function() {
	...
	core.onload = function() {
		...
		core.addEventListener( 'enterframe', function(){	//毎フレーム実行
			...
			core.tick += 1;
			// 常にゲットした具が右手に追従する
			for( i=0, getlen=gutetati.length; i<getlen; i++){	// 全ての具をチェックします。
				// gutetati[i].x = migite.x + 80 - i*10;
				// console.log(getlen);
			}
		});
		...
		kao.addEventListener('touchmove',function(e) {
			...
			hidarite.x =  this.x + 290 - this.width /2 - gutetati.length*10;	// 採れた具の分だけ左に寄って行きます。
		});
		
		kao.addEventListener('touchend',function(e) {
			migite.x =  this.x - 90 + this.width /2;
			hidarite.x =  this.x  - 210 + this.width /2 - gutetati.length*10;
			...
			for( i=0, len=gutati.length; i<len; i++){	// 全ての具をチェックします。
				if ( kao.x+300 <= gutati[i].x && gutati[i].x <= kao.x+400) {
					if ( this.intersect( gutati[i] ) ) {
						...
						score += 1;
						var gu_te = new GuTe( migite.x + 80, migite.y + 200, gutati[i].frame);	// バンズに挟まるオブジェクトを生成
					}
				}
			}
		});
